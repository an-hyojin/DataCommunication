...
public class EthernetLayer implements BaseLayer {
	...
	public boolean Send(byte[] input, int length) {
		...
		과제#4
		*/
		byte[] bytes = ObjToByte(m_sHeader, input, length);//헤더 붙여서
		this.GetUnderLayer().Send(bytes, length + 14);//하위로 전송. src 6 dst 6 type 2
		return true;
	}
	...
	public synchronized boolean Receive(byte[] input) {
		...
		int temp_type = byte2ToInt(input[12], input[13]); //type 확인
	
		/*
		과제#4
		 */
		if(temp_type==2){
			this.GetUpperLayer(0).Receive(null);//ACK가 왔을 경우 -> 상위레이어에 ACK왔다고 알림 (현재 송신중) 
		}else if(temp_type==1){
			if(chkAddr(input)){
				//나한테 온 패킷
		...
		}else if(temp_type==0xff&&(isBroadcast(input)&&!isMyPacket(input))){
			data = RemoveEthernetHeader(input, input.length);
			this.GetUpperLayer(0).Receive(data);
			return true;		
	...
	private boolean isBroadcast(byte[] bytes) {
		for(int i = 0; i< 6; i++)
			if (bytes[i] != (byte) 0xff)
				return false;
		if(bytes.length>13){//수신 때 확인할 것
			return (bytes[12]==(byte)0x00&&bytes[13] == (byte) 0xff);	
		}else{//송신 시
			return true;
		}
	}
