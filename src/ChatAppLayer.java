...
public class ChatAppLayer implements BaseLayer {
  ...
  /**/
    private void fragSend(byte[] input, int length) {
        ...
        int midLen = length-10;//이미 한번 송신 하였음
        
        m_sHeader.capp_totlen = intToByte2(10);
        for(i=1; i*10 < midLen; i++){//중간 송신
        	waitACK();
       	...
       	waitACK();
        m_sHeader.capp_type = (byte) (0x03);// 마지막 송신
        int lastlen = length%10==0?10:length%10;
        m_sHeader.capp_totlen = intToByte2(lastlen);
        bytes = new byte[lastlen];
        System.arraycopy(input, length-lastlen, bytes, 0, lastlen);
    ...
    public boolean Send(byte[] input, int length) {
        ...
        waitACK();
        if(length<=10){//단편화 할 필요가 없을 경우 헤더 0x00으로 전송
    ...
    public synchronized boolean Receive(byte[] input) {
        ...
        if(tempType == 0) { //데이터 하나일때
        	...
        	tempBytes = RemoveCappHeader(input, input.length);
        	System.arraycopy(tempBytes, 0, fragBytes, fragCount*10, len);
        	fragCount++;
        	if(tempType==3){//3일 경우 모은 데이터 상위 레이어로 전달
        		this.GetUpperLayer(0).Receive(fragBytes);
        	}
        }
            /*  과제   */
        this.GetUnderLayer().Send(null,0);
        return true;
    }
